<?php
  ...
  elseif ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['guardar'])) {
    ...
    if ($errores == "") {
      ...
      $cliente = $SQL->select("SELECT * FROM tblclientes ORDER BY idcliente DESC LIMIT 1");
      $SQL->close();
      $idcliente = $cliente[0][0];

      //print_r($cliente);
      header("location: producto.php?id=$idcliente");
    }
  ...
  require_once('../../views/ventas/index.view.php');
?>
